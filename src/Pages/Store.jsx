          ...
          <div className="w-[45%] h-auto">
            <div className="flex flex-col gap-y-3 h-[600px] mx-auto">
              <div className="big-one">
                <img className="big-one-img" src={images[imageIndex]} alt="Product" />
              </div>
              <div className="small-sec">
                {images.map((img, index) => (
                  <div key={index} className={`small-div ${imageIndex === index && "active-img"}`}>
                    <img className="small-img" onClick={() => setImageIndex(index)} src={img} alt={`Thumbnail ${index + 1}`} />
                  </div>
                ))}
              </div>
            </div>
